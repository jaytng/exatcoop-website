 ...
 * @since       1.6
 */
class MaximenuckModelMigration extends \Joomla\CMS\MVC\Model\ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$params	= \Joomla\CMS\Component\ComponentHelper::getParams('com_maximenuck');
	 ...
	 *
	 * @return  \Joomla\Data\DataObjectbaseQuery	A query object.
	 */
	...
	protected function getTypeOptionsFromXML($file, $component)
	{
		...
		if (!empty($menu['options']) && $menu['options'] == 'none')
		{
			...
			$o = new \Joomla\CMS\Object\CMSObject;
	...
	protected function getTypeOptionsFromMVC($component)
	{
		...
		if (is_dir($path))
		{
			$views = \Joomla\CMS\Filesystem\Folder::folders($path);
		}
		else
		{
			return false;
	...
	protected function getTypeOptionsFromLayouts($component, $view)
	{
		...
		if (is_dir($path))
		{
			$layouts = array_merge($layouts, \Joomla\CMS\Filesystem\Folder::files($path, '.xml$', false, true));
		}
		else
		{
			return $options;
		...
		$folders = \Joomla\CMS\Filesystem\Folder::folders(JPATH_SITE . '/templates', '', false, true);
		...
		foreach ($folders as $folder)
		{
			if (is_dir($folder . '/html/' . $component . '/' . $view))
			{
				...
				$templateLayouts = \Joomla\CMS\Filesystem\Folder::files($folder . '/html/' . $component . '/' . $view, '.xml$', false, true);
