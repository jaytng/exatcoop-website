...
class GalleryManager extends Widget
{
	...
	protected function ajax_upload()
	{
		...
		$resizeSettings = [
			...
			'original_image_resize_height' => $style === 'slideshow' ? ($field_data->get('original_image_resize_height', '') ? $field_data->get('original_image_resize_height', '') : null) : null
		];
	...
	protected function ajax_regenerate_images()
	{
		...
		if (is_array($items) && count($items))
		{
			foreach ($items as &$item)
			{
				...
				$sourceImageExists = $sourceImagePath && \JFile::exists($sourceImagePath);
				$originalImagePath = implode($ds, [JPATH_ROOT, $originalImage]);
				$originalImageExists = $originalImagePath && \JFile::exists($originalImagePath);
				...
				if ($style === 'slideshow')
				{
					if ($original_image_resize_height)
					{
